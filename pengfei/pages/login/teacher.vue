...
<script>
  ...
  import { get,deleteOne } from '~/static/js/api.js';
  export default {
    name: "teacher",
    async asyncData (){
    ...
    methods:{
      ...
      search:function () {
        ...
        if (that.searchVal.length!=0){
          that.loading=true;
          get('/teacher/name?name='+that.searchVal+'&currentPage='+that.currentPage+'&pageSize='+that.size).then(res=>{
            that.tableData=res.data;
            that.loading=false;
          })
        } else{
      ...
      deleteCurriculum:function (val) {
        // deleteOne('/deleteByidGive/'+val,'您确定要删除这条数据（无法恢复）？').then(res=>{
