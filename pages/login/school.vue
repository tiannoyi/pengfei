  ...
  export default {
    ...
    methods:{
      ...
      deleteThis:function (id) {
        deleteOne('/deleteSchool/'+id,'您确定删除该学校？',this);
      },
      //改变公告
      ...
      submitForm(formName) {
        ...
        that.$refs[formName].validate((valid) => {
          if (valid) {
            if (that.title=='添加学校'){
              axios.post("/insertSchool?schoolName="+that.ruleForm.schoolName,config).then(res=>{
              ...
              axios.put("/updateSchool?universityId="+this.ruleForm.universityId+'&schoolName='+this.ruleForm.schoolName).then(res=>{
                that.dialogFormVisible=false;
                that.refresh();
                that.$notify({
                  title: '成功',
