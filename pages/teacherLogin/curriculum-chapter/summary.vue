  ...
  <div style="width: 100%;">
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="视频名称" prop="videoName">
          <el-input v-model="videoData.videoName"></el-input>
        </el-form-item>
        <el-form-item label="视频时长" prop="videoDuration">
          <el-input type="textarea" v-model="videoData.videoDuration"></el-input>
        </el-form-item>
        <el-form-item label='选取上传视频'>
        	  <el-upload
        class="upload-demo"
        ref="upload"
       ...
       <el-button style="margin-left: 10px;" size="small" type="success" @click="submitData">修改数据（不上传图片）</el-button>
    ...
    <el-row type="flex" class="row-bg" justify="space-between" style="width: 100%;">
      <el-col :span="12">
        <el-button type="primary "  size="mini" @click="dialogsummany=true">修改小结</el-button>
        <el-button type="danger " size="mini"  @click="deleteKnowledge">删除小结</el-button>
        <el-button type="success "  size="mini" @click="addVideo">添加视频</el-button>
        <el-button type="success "  size="mini" @click="task">查看作业</el-button>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      submitData:function(){
      	post('/video/update',this.videoData).then(res=>{
      		this.successF();
      	})
      },
      //删除单个小结
      deleteKnowledge:function () {
        ...
        this.$confirm('此操作将永久删除该条小结, 是否继续?', '提示', {
          ...
          axios.delete('/Knowledge/'+that.knowledgeId).then(res=>{
            ...
            that.$store.commit('showChange');
            that.$router.push("/teacherLogin/curriculum-chapter")
          })
        }).catch(() => {
          this.$message({
            type: 'info',
      ...
      //添加视频跳转
      addVideo:function () {
        this.$router.push('/teacherLogin/addVideo')
      },
      //删除视频函数
      ...
      //查看作业
      task:function () {
        this.$router.push("/teacherLogin/curriculum-chapter/task");
      },
      //查看文章
      article:function () {
        this.$router.push("/teacherLogin/curriculum-chapter/article");
      }
    },
    mounted:function () {
      this.refresh()
